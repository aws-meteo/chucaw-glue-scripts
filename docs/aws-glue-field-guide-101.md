...
Mover datos GRIB desde Bronze a Platinum como Parquet, usando Glue Python Shell:
...
4. Bucket paths sugeridos en Platinum:
- `s3://chucaw-data-platinum-processed-725644097028-us-east-1-an/glue/scripts/`
- `s3://chucaw-data-platinum-processed-725644097028-us-east-1-an/glue/artifacts/`
...
- wheels transitive dependencies

## 5. Subir script y artefactos a S3

```powershell
aws s3 cp scripts/glue_jobs/bronze_to_platinum_parquet.py s3://chucaw-data-platinum-processed-725644097028-us-east-1-an/glue/scripts/bronze_to_platinum_parquet.py --region us-east-1

aws s3 cp dist/chucaw_preprocessor-0.1.0-py3-none-any.whl s3://chucaw-data-platinum-processed-725644097028-us-east-1-an/glue/artifacts/ --region us-east-1

aws s3 cp dist/glue-dependencies.gluewheels.zip s3://chucaw-data-platinum-processed-725644097028-us-east-1-an/glue/artifacts/ --region us-east-1
```
...
- Este job usa `Glue Python Shell` (`Command.Name = pythonshell`).
- En Python Shell no usas `DynamicFrame`; eso aplica al runtime Spark (`glueetl`).
- Aqui el procesamiento es con `xarray/cfgrib/pandas/pyarrow`, apropiado para GRIB -> Parquet modular.

Si mas adelante quieren catalogo Glue + transformaciones Spark, se puede crear un segundo job `glueetl` con `GlueContext` y DynamicFrames, leyendo el parquet producido.
